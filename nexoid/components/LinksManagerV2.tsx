...
import { ExternalLink, GripVertical, Loader2, Plus, Save, Trash2, ArrowUp, ArrowDown } from "lucide-react";
...
export function LinksManagerV2() {
    ...
    try {
      const normalized = links
        ...
        .map((item, idx) => ({
          ...
          order: idx, // Usar o índice como nova ordem
        }));
      ...
      // Reordenar links existentes
      if (existing.length > 0) {
        requests.push(
          fetch("/api/user/links-reorder", {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              links: existing.map((item) => ({ id: item.id, order: item.order })),
            }),
          })
        );
      }
      ...
      const savedLinkIds = new Set(normalized.map((l) => l.id).filter(Boolean));
      const originalLinkIds = new Set(links.map((l) => l.id).filter(Boolean));
      const deletedIds = [...originalLinkIds].filter((id) => !savedLinkIds.has(id));

      deletedIds.forEach((id) => {
        requests.push(fetch(`/api/user/links?id=${id}`, { method: "DELETE" }));
      });
      ...
      setSuccess("Links salvos e reordenados com sucesso.");
      await loadLinks();
    } catch (err: any) {
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={index === 0}
                className="inline-flex items-center rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-xs font-medium text-violet-600 transition hover:bg-violet-100 disabled:opacity-50 dark:border-violet-900/60 dark:bg-violet-950/40 dark:text-violet-200"
              >
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300">Os links salvos aparecem automaticamente no seu perfil público na ordem definida.</p>
          </div>
        </div>
      </div>
    </div>
